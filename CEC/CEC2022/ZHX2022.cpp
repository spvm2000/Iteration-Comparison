/*
  ...
  mex cec2022.cpp -DWINDOWS
  mex ZHX2022.cpp -DWINDOWS
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    {
		mexPrintf ("usage: f = cec22_func(x, func_num);\n");
		mexErrMsgTxt ("Error: this has error,D=2,10,20.");
    }
	m = mxGetN (prhs[0]);
	x = mxGetPr (prhs[0]);
...
void cec22_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "CEC/CEC2022/input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "CEC/CEC2022/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		{
			sprintf(FileName, "CEC/CEC2022/input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
